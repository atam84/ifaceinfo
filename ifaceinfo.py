...
class InterfacesInfos:
    ...
    def __get_ifaces_info(self, path):
        ...
        for iface in netInterfaces:
            ...
            if os.path.isdir(os.path.join(path, iface)):
                ...
                netIface['name'] = iface
                netIface['ip'] = self.__ip_address(iface)
                netIface['mask'] = self.__network_mask(iface)
                netIface['timestamp'] = self.__get_timestamp()
                if netIface['ip'] != '' and netIface['mask'] != '':
                    netIface['network_address'] = self.__network_address(netIface['ip'], netIface['mask'])
                else:
    ...
    def __network_mask(self, ifaceName):
        '''
        private methode
    ...
    def __network_address(self, ipaddr, mask):
        '''
        private methode
        used to calculate the network address
        '''
        x_ip = self.__ip2hex(ipaddr)
        x_mask = self.__ip2hex(mask)
        return self.__hex2ip(hex(int(x_ip, 16) & int(x_mask, 16)))

    def __ip2hex(self, addr):
        '''
        private methode
    ...
    def __reverse_ip(self, ipaddress):
        '''
    ...
    def __get_routes(self, routefile):
        ...
        for route in paramValue:
            ...
            _routing.append({
                'iface': _route[0],
                'x_dest': _route[1],
                'dest': self.__reverse_ip(self.__hex2ip(_route[1])),
                'x_gateway': _route[2],
                'gateway': self.__reverse_ip(self.__hex2ip(_route[2])),
                'flags': _route[3],
                'refcnt': _route[4],
                'use': _route[5],
                'metric': _route[6],
                'x_mask': _route[7],
                'mask': self.__reverse_ip(self.__hex2ip(_route[7])),
                'mtu': _route[8],
                'window': _route[9],
                'irtt': _route[10],
